...
<div class="homepage" style="max-width: 80rem; margin: 2rem auto;">
    ...
    <div class="content">
        ...
        @if ($today > $enrollDate && $enrollment->status !== 'DONE')
            <br><br><br>
            <a href="{{ route('finishMentoring', ['id' => $enrollment->id, 'userId' => $course->instructor_id]) }}">
                <button type="submit" class="btn btn-primary bg-white border border-primary rounded-3 ms-2 text-decoration: none;" style="color: #363636ff;">
                    Finish Mentoring
                </button>
            </a>
        @endif
        ...
        <div class="userData">
            <div style="display: flex;">
                <p>Tutee Name :</p>&nbsp;&nbsp;<p>{{ $mentor->name }}</p>
            </div>
            <div style="display: flex;">
                <p>Level :</p>&nbsp;&nbsp;<p>{{ $mentor->education_level }}</p>
            </div><br>
            <div>
                {{date('l, F jS, Y', strtotime($enrollment->date))}}
            </div>
