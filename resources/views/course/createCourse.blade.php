        ...
        <div class="preview-wrapper" style="flex: 1; display: flex; justify-content: center; align-items: center;">
            <div class="course-card" style="background-color: #f3f3f3; border: 1px solid #ccc; border-radius: 12px; padding: 12px; width: 240px; min-width: 240px; position: relative;">
                ...
                <div style="position: relative; border-radius: 10px; overflow: hidden;">
                    <img id="subjectImage" src="{{ isset($data) ? asset('Resources/' . $data->image) : asset('Resources/') }}"
                ...
                <div style="display: flex; align-items: center; gap: 8px; margin-top: 10px;">
                    ...
                    <span id="previewSubject" style="font-size: 14px; color: #555;">{{ isset($data) ? $data->subject : 'Subject' }}</span>
                </div>

                <div style="margin-top: 8px;">
                    <strong id="previewTitle" style="display: block; font-size: 16px; margin-bottom: 4px;">{{ isset($data) ? $data->title : 'Title' }}</strong>
                ...
                <div class="mb-3">
                    ...
                    <div style="display: flex; gap: 0.5rem; flex-wrap: wrap;">
                        ...
                        <button type="button" class="btn btn-outline-secondary subject-btn {{ isset($data) && $data->subject === $subject ? 'btn-active' : '' }}" style="width: fit-content;"
                            data-subject="{{ $subject }}" {{ isset($data) && $data->subject !== $subject ? 'disabled' : '' }}>{{ $subject }}</button>
                        @endforeach
                ...
                <div class="mb-3">
                    ...
                    <div style="display: flex; gap: 0.5rem; width: fit-content;">
                        ...
                        <button type="button" class="btn btn-outline-secondary session-btn {{ isset($data) && $data->session === $session ? 'btn-active' : '' }}"
                            data-session="{{ $session }}" {{ isset($data) && $data->session !== $session ? 'disabled' : '' }}>{{ $session }}</button>
                        @endforeach
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <div style="display: flex; gap: 0.5rem; width: fit-content;">
                        ...
                        <button type="button" class="btn btn-outline-secondary level-btn {{ isset($data) && $data->level === $level ? 'btn-active' : '' }}"
                            data-level="{{ $level }}" style="width: fit-content;" {{ isset($data) && $data->level !== $level ? 'disabled' : '' }}>{{ $level }}</button>
                        @endforeach
                    </div>
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content" style="display: flex;">
                            <div class="modal-header">
                                <h1 class="modal-title fs-5" id="modalDeleteLabel">Delete {{ $data->title }}?</h1>
